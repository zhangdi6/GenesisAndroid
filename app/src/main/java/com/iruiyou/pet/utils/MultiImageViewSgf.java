 ...
 */
public class MultiImageViewSgf extends LinearLayout {
    ...
    public MultiImageViewSgf(Context context) {
        super(context);
    }

    public MultiImageViewSgf(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
...
//            }
    ...
    private ImageView createImageView(final int position, final boolean isMultiImage) {
        ...
        if (!TextUtils.isEmpty(imagesList.get(position).getPath())) {
            url = imagesList.get(position).getPath();
        }
        ...
        // 加载网络图片/设置图片显示
        Glide.with(getContext()).load(BaseApi.baseUrlNoApi + url).into(imageView);
        return imageView;
    }
