...
package com.iruiyou.pet.activity.server.broadcast;
...
import com.iruiyou.pet.activity.server.network.async.AsyncTaskManager;
...
public class BroadcastManager {
    ...
    public void addActions(BroadcastReceiver receiver,String...action)
    {
        IntentFilter filter = new IntentFilter();
        if(action!= null && action.length > 0 )
        {
            ...
            mContext.registerReceiver(receiver,filter);
            receiverMap.put(Arrays.toString(action), receiver);
        }
    }
